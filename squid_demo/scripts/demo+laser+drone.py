#!/usr/bin/python3
import rospy, os, sys, rospkg, socket, threading
  ...
  if breakout:
    for i in range(100):#sometimes frames are missing
      ...
      if (cv2.waitKey(1) & 0xFF) == ord('q'):
            stopped()
    pos_min = 640-x-w#640-x-w/2 if 640-x-w/2>0 else 0
    pos_max = 640-x#640-x+w/2 if 640-x+w/2<640 else 640
    find_angle(int(pos_min),int(pos_max))
...
def find_angle(from_range,to_range):
    if len(distances)<to_range-from_range: return
    cropped_array = distances[from_range:to_range]
    array_length = len(cropped_array)
    distance_to_human = np.nanmin(cropped_array)
    index = from_range + np.nanargmin(cropped_array)
    angle_zero = (320 - index)/320*FOV/2
    print(f"Length: {array_length} Min distance: {distance_to_human} Index: {index} Angle: {angle_zero}")
    print(kill(angle_zero/1.2, distance_to_human*100-50))
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('scan', LaserScan, callback_dist)
    #rospy.spin()
    ...
    twist_msg = Twist()
    time.sleep(2)# sound_node needs time to wake up
